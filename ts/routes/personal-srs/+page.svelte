...
<script lang="ts">
    ...
    function addCard(): void {
        ...
        collection = { ...collection, cards: [card, ...collection.cards] };
        createFront = "";
        createBack = "";
        createSource = "";
    ...
    function review(rating: Rating): void {
    ...
    function deleteCard(cardId: string): void {
        collection = {
            ...
            reviewLog: collection.reviewLog.filter((entry) => entry.cardId !== cardId),
        };
        if (editingId === cardId) {
            editingId = null;
        }
        ...
        try {
            ...
            collection =
                importMode === "replace"
                    ...
                    : {
                          ...collection,
                          cards: [...incoming.cards, ...collection.cards],
                          reviewLog: [...incoming.reviewLog, ...collection.reviewLog],
                      };
            fileStatus = `${incoming.cards.length} cards imported.`;
        } catch (error) {
...
<main class="personal-srs">
    <section class="app-shell" aria-label="Personal spaced repetition app">
        ...
        <section class="workspace">
            {#if activeTab === "review"}
                <section class="tab-panel review-panel" aria-labelledby="review-title">
                    ...
                    {:else}
                        <div class="empty-state">
                            ...
                            {/if}
                            <button type="button" on:click={() => switchTab("create")}
                                >Create card</button
                            >
                        </div>
                    {/if}
            ...
            {:else if activeTab === "create"}
                <section class="tab-panel" aria-labelledby="create-title">
                    ...
                    <form class="card-form" on:submit|preventDefault={addCard}>
                        ...
                        <button type="submit" class="primary-action">Save card</button>
                        {#if createStatus}
            ...
            {:else if activeTab === "cards"}
                <section class="tab-panel" aria-labelledby="cards-title">
                    <div class="panel-heading">
                        ...
                        <span class="status-pill">{collection.cards.length} total</span>
                    </div>
                    ...
                    {#if editingId}
                        <form class="edit-panel" on:submit|preventDefault={saveEdit}>
                            <label>
                                <span>Deck</span>
                            ...
                            <label>
                                <span>Front</span>
                                <textarea bind:value={editFront} rows="4"></textarea>
                            </label>
                            <label>
                                <span>Back</span>
                        ...
                        {#each filteredCards as card}
                            <article>
                                ...
                                <div class="row-actions">
                                    <button
                                        type="button"
                                        on:click={() => startEditing(card)}>Edit</button
                                    >
                                    <button
                                        type="button"
                                        class="danger-link"
                            ...
                            <article>
                                ...
                                <div>
                                    <strong
                                        ...
                                        )?.front ?? "Deleted card"}</strong
                                    >
                                    <small
                                        >{formatDateTime(entry.reviewedAt)} - {entry.nextIntervalDays}d</small
                                    >
                                </div>
                            </article>
                        {:else}
